...
`--task` en `bb pr comment` marca el comentario como una **task** de
Bitbucket (aparece en la lista de tasks del PR, con estado `UNRESOLVED`) —
útil como filtro para detectar comentarios bloqueantes que hay que resolver
antes de mergear, a diferencia de un comentario normal que es solo
informativo.
